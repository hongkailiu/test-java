package tk.hongkailiu.test.app.tree.printer;

import tk.hongkailiu.test.app.tree.other.SimpleTree;
...
class BinaryTreePrinter {
    ...
    public static <T extends Comparable<T>> void printNode(SimpleTree<T> root) {
    ...
    private static <T extends Comparable<T>> void printNodeInternal(List<SimpleTree<T>> nodes,
        ...
        List<SimpleTree<T>> newNodes = new ArrayList<SimpleTree<T>>();
        for (SimpleTree<T> node : nodes) {
            if (node != null) {
    ...
    private static <T extends Comparable<T>> int maxLevel(SimpleTree<T> node) {
        if (node == null)
            return 0;
